...
void main() {
  group('TimeSyncStep Widget', () {
    ...
    group('Marker List State', () {
      ...
      testWidgets('fine tune +200ms updates synced marker time', (tester) async {
        await tester.pumpWidget(await createWidgetUnderTest(labels: ['verse', 'chorus']));
        await tester.pumpAndSettle();
        ...
        await tester.longPress(find.byKey(const ValueKey('markerSyncMarker_0')));
        await tester.pumpAndSettle();
        await tester.tap(find.text('+200ms'));
        await tester.pumpAndSettle();

        final markerTile = find.byKey(const ValueKey('markerSyncMarker_0'));
        expect(
          find.descendant(of: markerTile, matching: find.text('0:05.200')),
          findsOneWidget,
        );
      });
